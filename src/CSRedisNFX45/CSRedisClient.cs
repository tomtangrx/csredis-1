...
return 0", $"CSRedisPSubscribe{psubscribeKey}", "", trylong.ToString());
        ...
        /// <returns></returns>
        public long Incr(string key) => ExecuteScalar(key, (c, k) => c.Value.Incr(k));
        /// <summary>
        /// 将 key 所储存的值减去给定的增量值（increment）
        /// </summary>
        /// <param name="key">不含prefix前辍</param>
        /// <param name="value">增量值(默认=1)</param>
        /// <returns></returns>
        public long DecrBy(string key, long value = 1) => ExecuteScalar(key, (c, k) => c.Value.DecrBy(k, value));
        /// <summary>
        /// 将 key 所储存的值减去1
        /// </summary>
        /// <param name="key">不含prefix前辍</param> 
        /// <returns></returns>
        public long Decr(string key) => ExecuteScalar(key, (c, k) => c.Value.Decr(k));
        /// <summary>
        ...
        public bool SetEx(string key, long seconds, object value) {
            object redisValule = this.SerializeRedisValueInternal(value);
            return ExecuteScalar(key, (c, k) => c.Value.SetEx(k,seconds, redisValule)) == "OK";
        }
        /// <summary>
